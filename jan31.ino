...
unsigned long last_command_time = 0;  // NEW: Track last command received
bool command_timeout_active = false;   // NEW: Flag for timeout state

const unsigned long CONNECTION_CHECK_INTERVAL = 500; // Check every 500ms
const unsigned long COMMAND_TIMEOUT = 500;           // NEW: Stop motors after 500ms of no commands
...
void motor_callback(const void * msgin) {
  ...
  last_command_time = millis();  // NEW: Update timestamp on every command
  command_timeout_active = false; // NEW: Reset timeout flag
  
  for (int i = 0; i < 6 && i < msg->data.size; i++) {
    motor_pwm_values[i] = constrain(msg->data.data[i], -255, 255); // IMPROVED: Bounds checking
  }
}
...
void destroy_entities() {
  ...
  // IMPROVED: Free allocated memory
  if (motor_cmd_msg.data.data != NULL) {
    free(motor_cmd_msg.data.data);
    motor_cmd_msg.data.data = NULL;
  }
}
...
void setup() {
  ...
  last_command_time = millis(); // NEW: Initialize command timestamp
}
  ...
  switch (current_state) {
    ...
    case AGENT_AVAILABLE:
      if (create_entities()) {
        ...
        last_command_time = millis(); // NEW: Reset command timer when connected
    ...
    case AGENT_CONNECTED:
      // NEW: Check for command timeout - CRITICAL SAFETY FEATURE
      if (current_time - last_command_time > COMMAND_TIMEOUT) {
        if (!command_timeout_active) {
          stop_all_motors();
          command_timeout_active = true;
          digitalWrite(LED_PIN, HIGH); // Visual indicator of timeout
        }
      }
      ...
      // NEW: Only apply motors if not in timeout
      if (!command_timeout_active) {
        apply_motors();
      }
      break;
