...
const int pwmPins[6] = {2, 4, 6, 8, 10, 12};       // PWM control pins (PWM capable pins)
const int encoderAPins[6] = {27, 25, 40, 13, 15, 17};  // Interrupt-capable pins
const int encoderBPins[6] = {24, 26, 39, 41, 14, 16}; // Digital pins

// --- PWM Configuration ---
const int pwmFreq = 1000;      // PWM frequency (Hz)
const int pwmResolution = 255; // PWM resolution (8-bit = 0-255)

// --- Encoder Variables ---
volatile long encoderCounts[6] = {0, 0, 0, 0, 0, 0};
volatile int lastEncoded[6] = {0, 0, 0, 0, 0, 0};
bool invertEncoderCounting[6] = {false, false, false, false, false, false};
...
const int maxSpeed = 255;       // Max motor PWM (8-bit PWM)
...
void setup() {
  ...
  Serial.println();
  Serial.println("Teensy Six Motor Control System Ready");
  
  // Set PWM frequency for all PWM pins
  analogWriteFrequency(pwmPins[0], pwmFreq);
  analogWriteFrequency(pwmPins[1], pwmFreq);
  analogWriteFrequency(pwmPins[2], pwmFreq);
  analogWriteFrequency(pwmPins[3], pwmFreq);
  analogWriteFrequency(pwmPins[4], pwmFreq);
  analogWriteFrequency(pwmPins[5], pwmFreq);
  
  // Set PWM resolution
  analogWriteResolution(8); // 8-bit resolution (0-255)
  
  // Initialize motor pins and interrupts
  for (int i = 0; i < 6; i++) {
