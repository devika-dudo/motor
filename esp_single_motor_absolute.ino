...
void setup() {
  ...
  Serial.println("Commands: move <absolute_position>, reset, encode, stop");
  Serial.println("Example: move 1000 (go to position 1000), move 500 (go to position 500)");
}
...
void loop() {
  if (Serial.available()) {
    ...
    if (input.startsWith("move ")) {
      long absolutePosition = input.substring(5).toInt();
      // Check if we're already at the target position
      if (abs(encoderCount - absolutePosition) <= stopThreshold) {
        Serial.print("Already at target position: ");
        Serial.println(absolutePosition);
      } else {
        moveTicks(absolutePosition);
      }
    } else if (input == "stop") {
      stopMotor();
      ...
      Serial.println("Commands: move <absolute_position>, reset, encode, stop, f, b, s");
    }
  }
...
void moveTicks(long absoluteTarget) {
  long target = absoluteTarget;
  previousError = 0;
  integral = 0;
