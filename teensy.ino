...
const long COUNTS_PER_REV[NUM_MOTORS] = {
  480000, 400000, 400000, 30000, 80000, 240000
};

const int ENCODER_DIRECTION[NUM_MOTORS] = {
  -1, 1, -1, 1, 1, 1
};
const int MOTOR_DRIVE_DIRECTION[NUM_MOTORS] = {
  1, 1, 1, 1, -1, 1  
};
...
#define TIMER_PERIOD_US 1000    // 1ms = 1000us, this determines control frequency
...
//here we create a an array of those structs and length equal to number of motors 
MotorPID motors[NUM_MOTORS];
const float TOLERANCE_DEG = 0.5;
const int SETTLED_THRESHOLD = CONTROL_FREQUENCY; //its set to 1000?

int direct_pwm[NUM_MOTORS] = {0, 0, 0, 0, 0, 0}; //for direct joint control 

Encoder* encoders[NUM_MOTORS]; //these are pointers to encoder object
CytronMD* motor_drivers[NUM_MOTORS]; //the encoder library and cytron libraty has these classes 
...
//below two are macros -- 
#define RCCHECK(fn) { rcl_ret_t temp_rc = fn; if((temp_rc != RCL_RET_OK)){error_loop();}} //this runs a node and if it doesnt return ok, error loop will run
#define RCSOFTCHECK(fn) { rcl_ret_t temp_rc = fn; if((temp_rc != RCL_RET_OK)){}}//this runs a node and if it doesnt return ok nothing happens 

//if any node returns an error led at 13th pin will keep on blinking 
void error_loop(){
  while(1){
...
void mode_callback(const void * msgin) {
  ...
  if (strcmp(msg->data.data, "PID_CONTROL") == 0) {
    //only if pid control mode will it calculate error , 
    current_mode = MODE_PID;
    for (int i = 0; i < NUM_MOTORS; i++) {
      motors[i].target_angle = motors[i].current_angle;  // ← THIS LINE
      motors[i].integral = 0;
...
void joint_command_callback(const void * msgin) { //msgin is msg received from ros
  const std_msgs__msg__Float64MultiArray * msg = (const std_msgs__msg__Float64MultiArray *)msgin;
  for (size_t i = 0; i < NUM_MOTORS && i < msg->data.size; i++) {
    float new_target = msg->data.data[i] * (180.0 / M_PI);//converting to degrees
...
//here pid_control_pwm is set based on command and error 
void update_motor_pid(int motor_idx, float dt) {
  MotorPID &m = motors[motor_idx];//Create a reference m to the motor at index motor_idx, instead of using motors[motor_idx] again and again we use m
  ...
  if (abs(m.error) < TOLERANCE_DEG) {//if error reached within tolerance
    m.settled_count++;
    if (m.settled_count >= SETTLED_THRESHOLD) { //only if it sellted count increase and reaches settled_threshold will the motor stop
      m.pwm = 0;
    }
  } else {
    m.settled_count = 0;
...
// ================== TIMER CALLBACK ==================
//the function that runs every TIMER_PERIOD_US micro seconds
void timer_callback(rcl_timer_t * timer, int64_t last_call_time) {
  RCLC_UNUSED(last_call_time);//we dont use last_call_time ig, we could have removed this lines, but this removes the warning 1 unused parameter
  if (timer != NULL) {//timer is a structure that stores all timer info , checking whether timer is valid before doing anything
    const float dt = TIMER_DT;//fixed at since timer is supposed to run every 1ms 
    static int publish_counter = 0;
    publish_counter++;//so represents how many times timer callback runs
    
    // Read encoders
    for (int i = 0; i < NUM_MOTORS; i++) {
      motors[i].current_angle = countsToAngle(encoders[i]->read(), i); //encoders is from the library we use, it directly reads the encoder count
    }
    ...
    if (current_mode == MODE_PID) {
      for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        motor_drivers[i]->setSpeed(motors[i].pwm * MOTOR_DRIVE_DIRECTION[i]);      
        }
    } else { //if not mode_pid its set to direct pwm control 
      for (int i = 0; i < NUM_MOTORS; i++) {
    ...
    if (publish_counter >= PUBLISH_DECIMATION) {
      ...
      for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        joint_state_msg.velocity.data[i] = 0.0;//currently we dont have velocity state
      }
      ...
      RCSOFTCHECK(rcl_publish(&pub_joint_states, &joint_state_msg, NULL));//joint states published at 50hz
    }
  }
}
...
// ================== SETUP ==================
void setup() {
  pinMode(LED_PIN, OUTPUT); //10th pin set as output 
  digitalWrite(LED_PIN, HIGH); //if an led is connected to 13th pin -- it will remain on if this code is running 
// Joint 1
  motors[0].kp = 10.5;
  motors[0].ki = 0.0;
  motors[0].kd = 0.0;
  ...
  // Joint 4 ← needs different tuning due to low counts/rev
  motors[3].kp = 0.50;   // much lower Kp
  motors[3].ki = 0.0;
  motors[3].kd = 0.0;   // some damping
  ...
  delay(2000);
    //In micro-ROS / ROS 2, an allocator is: a mechanism that controls how memory is created (allocated) and destroyed (freed)


  allocator = rcl_get_default_allocator();
  RCCHECK(rclc_support_init(&support, 0, NULL, &allocator));//microros system is initialized using RCCCHECK 
  RCCHECK(rclc_node_init_default(&node, "teensy_motor_control", "", &support));//a node named teensy node is created using RCCHECK

  // ================== QoS Configuration ==================
  rmw_qos_profile_t qos_profile = rmw_qos_profile_default;//ros middle ware qos profile  set to something default 
  qos_profile.reliability = RMW_QOS_POLICY_RELIABILITY_RELIABLE;//and reliability is set to reliable means Reliable QoS = “Make sure the message reaches the receiver, no matter what”, if its lost it will send again till the subscriber recieves it 
  //we didnt use best effort reliability because it just send it once doesnt care if its received or not 
  qos_profile.durability = RMW_QOS_POLICY_DURABILITY_VOLATILE; //volatile means only send messages to subscribiers that are currently connected , if someone joins late they wont reveive the message,
  //another durability is transient local --- it stores last message and gives it to new subscribers 
  qos_profile.history = RMW_QOS_POLICY_HISTORY_KEEP_LAST;//store only the most recent N messages , if it was keep all it stores everything till memory fills
  qos_profile.depth = 10;
//setting up memory for JointState message 
  ...
  RCCHECK(rclc_timer_init_default(&timer, &support, RCL_US_TO_NS(TIMER_PERIOD_US), timer_callback));//creates a timer that calles timer_callback every TIMER_PERIOD_US times 
  RCCHECK(rclc_executor_init(&executor, &support.context, 4, &allocator));//creates an executor that will manage timers subscribers etc
  //things that it needs to handle is 4 --like timers, subscribers (i think publisher doesnt need to handled like theres no need for active listening and all)
  ...
  digitalWrite(LED_PIN, LOW); //if it comes here led off?---thats is after setup led off 
}
//executor is part of ros that runs callbacks , it reacts to timer callbacks and data coming of subscribed topics 
void loop() {
  RCSOFTCHECK(rclc_executor_spin_some(&executor, RCL_MS_TO_NS(10)));//executor checks for events and run callbacks for upto 10ms 
  //spin_some checks if timer is ready and checks if messages arrived and run corresponding callbacks 
  //ok so this runs every 10ms --- because ros uses ns we convert it , so callbacks are processed every 10ms
}
